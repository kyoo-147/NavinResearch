...
function productConsole(kind = "overview") {
  const surfaces = {
    overview: ["Department overview", "Review required", [["Operations", "READY"], ["Research", "WAITING"], ["Communications", "REVIEW"]]],
    departments: ["Department control room", "Human governed", [["Responsibility", "NAMED"], ["Workflow boundary", "DEFINED"], ["Authority", "HUMAN"]]],
    agents: ["Agent state", "Role bounded", [["Research coordinator", "WAITING"], ["Operations steward", "READY"], ["Release reviewer", "HUMAN"]]],
    workflow: ["Execution route", "Approval pending", [["Intake", "RECORDED"], ["Delegation", "BOUNDED"], ["Review", "REQUIRED"]]],
    runtime: ["Runtime lanes", "Not released", [["Worker state", "VISIBLE"], ["Capability", "DECLARED"], ["Failure", "STOPPED"]]],
    observability: ["Event trace", "Question open", [["request.received", "KNOWN"], ["context.missing", "OPEN"], ["human.review", "WAIT"]]],
    memory: ["Context surface", "Retention unknown", [["Task context", "SCOPED"], ["Provenance", "ATTACHED"], ["Deletion", "UNANNOUNCED"]]],
    approvals: ["Decision record", "Human authority", [["Proposal", "RECORDED"], ["Evidence", "ATTACHED"], ["Decision", "PENDING"]]],
    integrations: ["Connector contract", "Category only", [["Capability", "DECLARED"], ["Credential", "BOUNDED"], ["Return", "RECORDED"]]],
    developers: ["Developer surface", "No public API", [["Schema", "ILLUSTRATIVE"], ["CLI", "NOT RELEASED"], ["SDK", "NOT ANNOUNCED"]]],
    enterprise: ["Enterprise evidence", "Requirements only", [["Scale envelope", "UNKNOWN"], ["Reliability", "UNVERIFIED"], ["Deployment", "UNANNOUNCED"]]],
    security: ["Control boundary", "No certification", [["Identity", "REQUIRED"], ["Action record", "DESIGN INTENT"], ["Assurance", "UNVERIFIED"]]]
  };
  const [title, status, rows] = surfaces[kind] || surfaces.overview;
  return `<section class="sd-product-console" data-surface="${e(kind)}" aria-label="Illustrative Sandora ${e(title)} interface study; not a live product"><header><div><i aria-hidden="true">S</i><strong>SANDORA / ${e(title.toUpperCase())}</strong></div>${badge("INTERFACE STUDY")}</header><div class="sd-console-layout"><aside aria-label="Illustrative product navigation"><span>01 Overview</span><span>02 Roles</span><span>03 Routes</span><span>04 Evidence</span></aside><div class="sd-console-main"><div class="sd-console-title"><div><span>OPERATING SURFACE</span><h3>${e(title)}</h3></div><strong>${e(status)}</strong></div><div class="sd-console-rows">${rows.map(([label, state], index) => `<div><span>${String(index + 1).padStart(2, "0")}</span><strong>${e(label)}</strong><b>${e(state)}</b></div>`).join("")}</div><div class="sd-console-trace" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div></div></div><div class="sd-console-foot"><span>CONCEPT-STAGE PRODUCT SURFACE</span><span>NO RUNTIME OR AVAILABILITY CLAIM</span></div></section>`;
}
...
function home(product, site, page) {
  ...
  return `<div class="sd-modules sd-home">
    ...
    ${evidence(product)}
    <section class="sd-factory-intro sd-light"><header><span>OPERATING PRINCIPLE / 01</span><h2>Responsibility before automation.</h2></header><div><p>${e(product.intro)}</p></div></section>
    ...
    <section class="sd-home-capabilities sd-light"><header><span>PRODUCT SURFACES / 02</span><h2>Six surfaces. One operating model.</h2></header><div class="sd-capability-grid"><a href="/departments/"><b>01</b><h3>Departments</h3><p>Named responsibility and outcome.</p></a><a href="/agents/"><b>02</b><h3>Agents</h3><p>Bounded roles with visible state.</p></a><a href="/workflows/"><b>03</b><h3>Workflows</h3><p>Recorded stages and handoffs.</p></a><a href="/memory/"><b>04</b><h3>Memory</h3><p>Scoped context with provenance.</p></a><a href="/approvals/"><b>05</b><h3>Approvals</h3><p>Human-owned state transitions.</p></a><a href="/observability/"><b>06</b><h3>Observability</h3><p>Events, unknowns, and intervention.</p></a></div></section>
    ...
    <section class="sd-technical sd-light"><header><span>PRODUCT RECORD / 04</span><h2>What the record must preserve.</h2></header><div class="sd-tech-layout">${productConsole("overview")}<div><article><b>01</b><h3>State</h3><p>Runtime lanes expose what is ready, waiting, or stopped.</p><a href="/runtime/">Runtime ${arrow}</a></article><article><b>02</b><h3>Context</h3><p>Integrations declare capability, credential, and return boundaries.</p><a href="/integrations/">Integrations ${arrow}</a></article><article><b>03</b><h3>Evidence</h3><p>Research questions remain distinct from proof.</p><a href="/research/">Research ${arrow}</a></article><article><b>04</b><h3>Contract</h3><p>Developer material stays separate from a live API or SDK.</p><a href="/developers/">Developers ${arrow}</a></article></div></div></section>
    ...
    <section class="sd-home-close sd-light"><span>HUMAN ENTRY / 06</span><h2>Bring the workflow.<br>Keep the boundary.</h2><p>A conversation creates no access or deployment commitment.</p><a href="${e(site.primaryCta.href)}"${ext(site.primaryCta.href)}>DISCUSS SANDORA ${arrow}</a></section>
  </div>`;
}

function routeHero(page, variant = "light", surface = "") {
  const visual = surface ? productConsole(surface) : media(page, "sd-media--route-hero", true);
  const solo = visual ? "" : " sd-route-hero--solo";
  return `<section class="sd-route-lead sd-route-hero sd-route-hero--${e(variant)}${solo}"><div><span>${e(page.eyebrow)}</span><h1>${e(page.headline)}</h1><p>${e(page.lede)}</p><div class="sd-boundary-line"><span>OWNER <b>HUMAN</b></span><span>STATE <b>EXPLICIT</b></span><span>EVIDENCE <b>BOUNDED</b></span></div></div>${visual}</section>`;
}
...
function memory(page) {
  ...
  return `<div class="sd-runbook sd-memory">${routeHero(page, "cobalt", "memory")}<div class="sd-agent-marquee" aria-hidden="true">CONTEXT · PROVENANCE · RETENTION · CONTEXT · PROVENANCE ·</div><section class="sd-memory-bands">${layers.map(([title, body], index) => `<article><div><span>0${index + 1} / MEMORY LAYER</span><h2>${e(title)}</h2><p>${e(body)}</p>${badge(index === 3 ? "NOT ANNOUNCED" : "DESIGN INTENT")}</div><div class="sd-memory-core"><i></i><i></i><i></i><strong>${String(index + 1).padStart(2, "0")}</strong></div></article>`).join("")}</section><section class="sd-agent-research"><div><span>RESEARCH BRIDGE</span><h2>Memory needs a method.</h2></div><div><p>Evaluation must distinguish retrieval, persistence, adaptation, forgetting, and human correction. No continual-learning result is claimed.</p><a href="/research/">RESEARCH QUESTIONS ${arrow}</a></div></section>${distribution()}</div>`;
}
...
function pricing(page) {
  ...
  return `<div class="sd-availability-board sd-pricing-page sd-light"><section class="sd-route-lead sd-pricing-head"><span>PACKAGE SHAPES / PUBLIC STATUS</span><h1>Pricing is not announced.</h1><p>${e(page.lede)}</p></section><section class="sd-plan-grid">${plans.map(([number, name, purpose, points], index) => `<article><span>${number}</span><header><h2>${name}</h2><strong>CONCEPT PACKAGE</strong></header><p>${purpose}</p><hr><ul>${points.map((point) => `<li>${e(point)}</li>`).join("")}</ul><a href="${index === 0 ? "/product/" : "/contact/"}">${index === 0 ? "READ THE MODEL" : "DISCUSS REQUIREMENTS"} ${arrow}</a></article>`).join("")}</section><section class="sd-plan-compare"><header><span>COMPARISON / STATUS ONLY</span><h2>Compare the questions, not entitlements.</h2></header><div role="region" aria-label="Illustrative package comparison" tabindex="0"><table><thead><tr><th>Surface</th><th>Observer</th><th>Builder</th><th>Steward</th><th>Team</th><th>Organization</th><th>Enterprise</th></tr></thead><tbody><tr><th scope="row">Concept documentation</th><td>Direction</td><td>Direction</td><td>Direction</td><td>Direction</td><td>Direction</td><td>Direction</td></tr><tr><th scope="row">Workflow requirements</th><td>—</td><td>Discuss</td><td>Discuss</td><td>Discuss</td><td>Discuss</td><td>Discuss</td></tr><tr><th scope="row">Governance requirements</th><td>—</td><td>—</td><td>Discuss</td><td>Discuss</td><td>Discuss</td><td>Discuss</td></tr><tr><th scope="row">Price / entitlement</th><td colspan="6">NOT ANNOUNCED</td></tr></tbody></table></div></section><section class="sd-plan-faq"><header><span>FAQ</span><h2>Unknown values stay blank.</h2></header><div><details><summary>Is Sandora available?</summary><p>No public product, access program, or release is announced.</p></details><details><summary>Are these purchasable plans?</summary><p>No. They are package shapes used to organize future questions.</p></details><details><summary>What does a discussion create?</summary><p>A human conversation only—not access, pricing, deployment, or service commitments.</p></details></div></section>${handoff("No price, allowance, plan entitlement, discount, or guarantee is approved.", "/contact/", "Discuss future requirements")}</div>`;
}
