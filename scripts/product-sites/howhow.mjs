...
function layoutSectionMarkup(page, layout) {
  ...
  if (layout === "workflow") return `<ol class="product-workflow" data-site-frame="howhow-recipe" aria-label="HowHow research recipe">${sections.map((section, index) => item(section, index, renderSectionPoints(section, layout), "li")).join("")}</ol>`;
  if (layout === "ledger") return `<aside class="product-ledger" data-site-frame="howhow-claim-ledger" aria-label="HowHow evidence register">${sections.map((section, index) => item(section, index, renderSectionPoints(section, layout))).join("")}</aside>`;
